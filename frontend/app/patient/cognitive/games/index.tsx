// app/patient/cognitive/games/index.tsx
...
export default function GamesDashboard() {
  ...
  const easyCnt = gamePlan.assignments.filter(a => a.difficulty === 'easy').length;
  const mediumCnt = gamePlan.assignments.filter(a => a.difficulty === 'medium').length;
  const hardCnt = gamePlan.assignments.filter(a => a.difficulty === 'hard').length;
          ...
          <View className="flex-row gap-2 px-6 py-4">
            {easyCnt > 0 && (
              <View className="flex-row items-center gap-1.5 bg-green-100 rounded-full px-3 py-1.5">
                ...
                <Text className="text-xs font-semibold text-green-700">{easyCnt} Easy</Text>
              </View>
            )}
            {mediumCnt > 0 && (
              <View className="flex-row items-center gap-1.5 bg-amber-100 rounded-full px-3 py-1.5">
                <View className="w-2 h-2 rounded-full bg-amber-500" />
                <Text className="text-xs font-semibold text-amber-700">{mediumCnt} Medium</Text>
              </View>
            )}
            {hardCnt > 0 && (
              <View className="flex-row items-center gap-1.5 bg-red-100 rounded-full px-3 py-1.5">
                <View className="w-2 h-2 rounded-full bg-red-500" />
                <Text className="text-xs font-semibold text-red-700">{hardCnt} Hard</Text>
              </View>
            )}
          </View>
            ...
            {gamePlan.assignments.map(assignment => {
              ...
              return (
                <TouchableOpacity
                  ...
                  activeOpacity={0.7}
                  className={`rounded-3xl border p-5 ${c.bg} ${c.border}`}
                >
                  <View className="flex-row items-start gap-4">
                    <View className={`w-14 h-14 rounded-2xl items-center justify-center ${c.icon}`}>
                    ...
                    <View className="flex-1">
                      ...
                      <View className="h-1.5 bg-white rounded-full overflow-hidden">
                        <View
                          className={`h-full rounded-full ${assignment.difficulty === 'easy' ? 'bg-green-400' :
                              assignment.difficulty === 'medium' ? 'bg-amber-400' : 'bg-red-400'
                            }`}
