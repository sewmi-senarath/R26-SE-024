import { RELATION_OPTIONS } from "@/src/constants/PatientFormConstants";
import {
    FamilyMember,
    LifeEvent,
    Step2Data,
...
export default function Step2PersonalMemories({ data, onChange }: Step2Props) {
  ...
  const handlePickFamilyPhoto = async (id: string) => {
    ...
    if (!result.canceled && result.assets?.[0]) {
      ...
      if (durableUri) {
        handleUpdateFamilyMember(id, "photo", durableUri);
      }
    }
  };
  ...
  const handleAddCountry = () => {
    onChange({ countriesLived: [...data.countriesLived, ""] });
  };

  const handleRemoveCountry = (index: number) => {
    onChange({
      countriesLived: data.countriesLived.filter(
        (_, itemIndex) => itemIndex !== index,
      ),
    });
  };

  const handleUpdateCountry = (index: number, country: string) => {
    onChange({
      countriesLived: data.countriesLived.map((item, itemIndex) =>
        itemIndex === index ? country : item,
      ),
    });
  };

  const handleAddOccupation = () => {
    onChange({ occupations: [...data.occupations, ""] });
  };

  const handleRemoveOccupation = (index: number) => {
    onChange({
      occupations: data.occupations.filter(
        (_, itemIndex) => itemIndex !== index,
      ),
    });
  };

  const handleUpdateOccupation = (index: number, occupation: string) => {
    onChange({
      occupations: data.occupations.map((item, itemIndex) =>
        itemIndex === index ? occupation : item,
      ),
    });
  };
    ...
    <View className="gap-4">
      ...
      <View>
        ...
        <Text className="text-xs text-gray-600 mb-2">
          The city or town the patient calls home
        </Text>
        <View className="flex-row items-start bg-white rounded-lg px-4 py-3 shadow-sm border border-gray-200">
          ...
          <TextInput
            ...
            placeholderTextColor="#d1d5db"
            value={data.hometown}
            onChangeText={(text) => onChange({ hometown: text })}
          />
        </View>
      </View>
      ...
      <View className="z-30">
        ...
        {data.countriesLived.map((country, index) => (
          <View
            key={`country-${index}`}
          ...
          >
            <View className="flex-1 flex-row items-start bg-white rounded-lg px-4 py-3 shadow-sm border border-gray-200">
              ...
              <TextInput
                ...
                placeholderTextColor="#d1d5db"
                value={country}
                onChangeText={(text) => handleUpdateCountry(index, text)}
              />
            </View>
            <TouchableOpacity
              onPress={() => handleRemoveCountry(index)}
              className="bg-red-100 rounded-lg p-2"
            >
              <Ionicons name="close" size={20} color="#ef4444" />
            </TouchableOpacity>
        ...
        {data.countriesLived.length === 0 && (
          <View className="bg-blue-50 rounded-lg p-3 border border-blue-200">
            <Text className="text-sm text-blue-700 text-center">
              Add countries to get started
            </Text>
          </View>
        )}
      </View>
      ...
      <View>
          ...
          >
            <View className="flex-1 flex-row items-start bg-white rounded-lg px-4 py-3 shadow-sm border border-gray-200">
              ...
              <TextInput
                ...
                placeholderTextColor="#d1d5db"
                value={occupation}
                onChangeText={(text) => handleUpdateOccupation(index, text)}
              />
            </View>
            <TouchableOpacity
              onPress={() => handleRemoveOccupation(index)}
              className="bg-red-100 rounded-lg p-2"
            >
              <Ionicons name="close" size={20} color="#ef4444" />
            </TouchableOpacity>
        ...
        {data.occupations.length === 0 && (
          <View className="bg-blue-50 rounded-lg p-3 border border-blue-200">
            <Text className="text-sm text-blue-700 text-center">
              Add occupations to get started
            </Text>
          </View>
        )}
      </View>
