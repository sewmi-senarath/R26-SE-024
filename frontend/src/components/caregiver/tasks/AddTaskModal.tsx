...
export const AddTaskModal: React.FC<Props> = ({
  ...
  // ── Updated handleAdd — calls real backend ─────────────────────────────
  const handleAdd = async () => {
    if (!title.trim() || !patientId) return;
    const patient = MOCK_PATIENTS.find((p) => p.id === patientId)!;

    const taskPayload: Omit<CaregiverTask, 'id'> = {
      title:           title.trim(),
      patientName:     patient.name,
      patientInitials: patient.initials,
      patientColor:    patient.color,
    ...
    try {
      ...
      onAdd(savedTask); // ← passes task with real MongoDB _id
      reset();
      onClose();
    } catch (error) {
                ...
                {selectedPatient ? (
                  <View style={{ flexDirection: 'row', alignItems: 'center', gap: 8 }}>
                    <View style={{
                      ...
                      backgroundColor: selectedPatient.color + '22',
                      alignItems: 'center', justifyContent: 'center',
                    }}>
                      <Text style={{
                        fontSize: 8, fontWeight: '800', color: selectedPatient.color,
                      }}>
            ...
            {showPatients && (
              <View style={{
                ...
                {MOCK_PATIENTS.map((p) => (
                  <TouchableOpacity
                    key={p.id}
                    onPress={() => { setPatientId(p.id); setShowPatients(false); }}
                    style={{
                      flexDirection: 'row', alignItems: 'center',
                      gap: 10, padding: 12,
                      backgroundColor: patientId === p.id
                  ...
                  >
                    ...
                    <Text style={{
                      ...
                      fontWeight: patientId === p.id ? '700' : '400',
                    }}>
                      {p.name}
                    </Text>
                    {patientId === p.id && (
          ...
          <View style={{ flexDirection: 'row', gap: 10, paddingTop: 8 }}>
            ...
            <TouchableOpacity
              ...
              style={{
                ...
                backgroundColor:
                  !title.trim() || !patientId || submitting
                    ? Colors.border
                    : Colors.primary,
              }}
            >
